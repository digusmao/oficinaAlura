...
corpo.addEventListener("mousemove", function (evento) {
    ...
    let pocicaoBotao = botao.getBoundingClientRect();

    const distanciaLimite = 100;

    let centroBotaoX = posicaoBotao.left + posicaoBotao.width / 2;
    let centroBotaoY = posicaoBotao.top + posicaoBotao.heigt / 2;

    let distanciaX = mouseX - centroBotaoX;
    let distanciaY = mouseY - centroBotaoY;

    if (Math.abs(distanciaX) < distanciaLimite &&
        Math.abs(distanciaY) < distanciaLimite
        ...
        let novaPosicaoY = Math.min(window.innerHeight - posicaoBotao.heigt, Math.max(0, botao.offsetTop + moverY));
        botao.style.position = "absolute";
        botao.style.left = novaPosicaoX + "px";
        botao.style.left = novaPosicaoY + "px";
